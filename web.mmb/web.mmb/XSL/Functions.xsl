...
<xsl:template name="GetSellingPrice">
	...
	<xsl:param name="SeparateDecimals" select="'False'"/>
	...
	<!-- display decimals in seperate element -->
	<xsl:variable name="DisplayValue" select="format-number($NewValue, $Format)" />
	<xsl:variable name="PriceLength" select="string-length($DisplayValue)" />
	
	<xsl:choose>
		<xsl:when test="$SeparateDecimals = 'True'">
			<xsl:value-of select="$Sign"/>
			<xsl:if test="$CurrencySymbolPosition='Prepend'">
				<xsl:value-of select="$Currency"/>
			</xsl:if>
			<xsl:value-of select="substring($DisplayValue, 1, $PriceLength - 3)"/>
			<span class="decimal">
				<xsl:value-of select="substring($DisplayValue, $PriceLength - 2)"/>
			</span>			
...
<xsl:template name="ShortMonth">
	...
	<xsl:choose>
		<xsl:when test="$MonthNumber=1"><xsl:text>Jan</xsl:text></xsl:when>
		<xsl:when test="$MonthNumber=2"><xsl:text>Feb</xsl:text></xsl:when>
		<xsl:when test="$MonthNumber=3"><xsl:text>Mar</xsl:text></xsl:when>
		<xsl:when test="$MonthNumber=4"><xsl:text>Apr</xsl:text></xsl:when>
		<xsl:when test="$MonthNumber=5"><xsl:text>May</xsl:text></xsl:when>
		<xsl:when test="$MonthNumber=6"><xsl:text>Jun</xsl:text></xsl:when>
		<xsl:when test="$MonthNumber=7"><xsl:text>Jul</xsl:text></xsl:when>
		<xsl:when test="$MonthNumber=8"><xsl:text>Aug</xsl:text></xsl:when>
		<xsl:when test="$MonthNumber=9"><xsl:text>Sep</xsl:text></xsl:when>
		<xsl:when test="$MonthNumber=10"><xsl:text>Oct</xsl:text></xsl:when>
		<xsl:when test="$MonthNumber=11"><xsl:text>Nov</xsl:text></xsl:when>
		<xsl:when test="$MonthNumber=12"><xsl:text>Dec</xsl:text></xsl:when>
	</xsl:choose>
...
<xsl:template name="AbbreviateMealBasis">
    ...
    <xsl:choose>
        ...
        <xsl:otherwise><xsl:value-of select="$MealBasisText"/></xsl:otherwise>
    </xsl:choose>
</xsl:template>
...
<xsl:template name="AbbreviateMonth">
    ...
    <xsl:choose>
        ...
        <xsl:otherwise><xsl:value-of select="$MonthText"/></xsl:otherwise>
    </xsl:choose>
</xsl:template>
    ...
    <xsl:if test="contains($LinkText,'[') and contains($LinkText,'](') and contains($LinkText,')')">
        ...
        <a>
            ...
            <xsl:if test="$AddClass!=''">
                <xsl:attribute name="class"><xsl:value-of select="$AddClass"/></xsl:attribute>
            </xsl:if>
...
<xsl:template name="StarRating">
	...
	<xsl:param name="alternative" select="'false'"/>
