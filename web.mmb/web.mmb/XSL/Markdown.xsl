...
<xsl:template name="Markdown">
	...
	<xsl:choose>
		...
		<xsl:when test="starts-with($text,'##')">
			<h4><xsl:call-template name="MarkdownBlock"><xsl:with-param name="block" select="substring($line,3)"/></xsl:call-template></h4>
		</xsl:when>
		
		<xsl:when test="starts-with($text,'#')">
			<h3><xsl:call-template name="MarkdownBlock"><xsl:with-param name="block" select="substring($line,2)"/></xsl:call-template></h3>
		</xsl:when>
	
		<xsl:when test="starts-with($text,'- ')">
			<li><xsl:call-template name="MarkdownBlock"><xsl:with-param name="block" select="substring($line, 3)"/></xsl:call-template></li>
		</xsl:when>	
		...
		<xsl:otherwise>
			<xsl:if test="$line!=''">
				<p><xsl:call-template name="MarkdownBlock"><xsl:with-param name="block" select="$line"/></xsl:call-template></p>
			</xsl:if>
		</xsl:otherwise>
	...
	<xsl:choose>
		...
		<xsl:when test="contains($block,'**') and contains(substring-after($block,'**'),'**')">
			...
			<xsl:call-template name="MarkdownBlock">
				<xsl:with-param name="block" select="$beforefirststar"/>
			</xsl:call-template>
			
			<strong>
				<xsl:call-template name="MarkdownBlock">
					<xsl:with-param name="block" select="$emtext"/>
				</xsl:call-template>
			</strong>
			<xsl:call-template name="MarkdownBlock">
		...
		<xsl:when test="contains($block,'*') and contains(substring-after($block,'*'),'*')">
			...
			<em><xsl:value-of select="$emtext"/></em>
			<xsl:call-template name="MarkdownBlock">
				<xsl:with-param name="block" select="$aftersecondstar"/>
			</xsl:call-template>
		</xsl:when>
		...
		<xsl:when test="contains($block,'[') and contains($block,'](') and contains($block,')')">
			...
			<xsl:call-template name="MarkdownBlock">
				<xsl:with-param name="block" select="substring-after($block,concat($inroundbracket,')'))"/>
			</xsl:call-template>
		...
		<xsl:when test="contains($block,'(#') and contains($block,')') and not(contains($block,' '))">
			...
			<a id="{$anchorid}"><xsl:text> </xsl:text></a>
