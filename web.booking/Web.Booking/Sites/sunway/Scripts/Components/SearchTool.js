...
export default class SearchTool extends React.Component {
    constructor(props) {
        ...
        this.isValidArrivalAirport = this.isValidArrivalAirport.bind(this);
        this.getRegion = this.getRegion.bind(this);
        this.renderMoreDestinations = this.renderMoreDestinations.bind(this);
        this.changeSearchMode = this.changeSearchMode.bind(this);
        this.getCalendarMessage = this.getCalendarMessage.bind(this);
    ...
    displayAutoSuggest() {
        ...
        const displayAutoSuggest = this.props.destinationSearchOptions.displayAutoComplete
            ...
            && currentSearchMode === this.searchModes.HOTEL;
    ...
    displayDestinationDropdown() {
        const currentSearchMode = this.props.searchDetails.SearchMode;
        return currentSearchMode !== this.searchModes.HOTEL;
    }
    getCalendarMessage() {
        const currentSearchMode = this.props.searchDetails.SearchMode;
        let message = '';
        if (this.props.calendarMessages.length > 0) {
            switch (currentSearchMode) {
                case this.searchModes.FLIGHT_PLUS_HOTEL:
                    for (let i = 0; i < this.props.calendarMessages.length; i++) {
                        if (this.props.calendarMessages[i].context === 'package') {
                            message = this.props.calendarMessages[i]['Calendar Text'];
                            break;
                        }
                    }
                    break;
                case this.searchModes.FLIGHT:
                    for (let i = 0; i < this.props.calendarMessages.length; i++) {
                        if (this.props.calendarMessages[i].context === 'flight-only') {
                            message = this.props.calendarMessages[i]['Calendar Text'];
                            break;
                        }
                    }
                    break;
                case this.searchModes.HOTEL:
                    for (let i = 0; i < this.props.calendarMessages.length; i++) {
                        if (this.props.calendarMessages[i].context === 'hotel-only') {
                            message = this.props.calendarMessages[i]['Calendar Text'];
                            break;
                        }
                    }
    ...
    getTabProps() {
        let selectedTab = this.props.searchDetails.SearchMode;
        if (this.state.showMoreDestinations) {
            selectedTab = 'More';
        }
        const tabProps = {
            selectedTab,
            ...
            tabProps.tabs.push(
                {
                    ...
                    iconLocation: 'Tab',
                    onClick: () =>
                        this.changeSearchMode(this.searchModes.FLIGHT_PLUS_HOTEL),
                });
        }
            ...
            tabProps.tabs.push(
                {
                    ...
                    iconClass: 'plane',
                    iconLocation: 'Tab',
                    onClick: () => this.changeSearchMode(this.searchModes.FLIGHT),
                });
        }
            ...
            tabProps.tabs.push(
                {
                    ...
                    iconClass: 'hotel',
                    iconLocation: 'Tab',
                    onClick: () => this.changeSearchMode(this.searchModes.HOTEL),
                });
        }

        if (this.props.moreDestinations
            && this.props.moreDestinations.hasOwnProperty('Include More Options')
            && this.props.moreDestinations['Include More Options']) {
            tabProps.tabs.push(
                {
                    name: this.props.sidebarDisplay ? '' : 'More',
                    value: 'More',
                    iconClass: 'more',
                    iconLocation: 'Tab',
                    onClick: () => this.setState({ showMoreDestinations: true }),
                });
        }
    ...
    changeSearchMode(searchMode) {
        this.updateProperty('SearchMode', searchMode);
        this.setState({ showMoreDestinations: false });
    }
    ...
    isValidArrivalAirport(departureAirpotId, arrivalAirportId) {
        const airportRoutes = this.getDepartureAirportRoutes(departureAirpotId);
        let isValidArrivalAirport = false;
        if (airportRoutes.ArrivalAirportIDs) {
            for (let i = 0; i < airportRoutes.ArrivalAirportIDs.length; i++) {
                const arrivalAirport = airportRoutes.ArrivalAirportIDs[i];
                if (arrivalAirport === arrivalAirportId) {
                    isValidArrivalAirport = true;
                    break;
                }
            }
        }
    ...
    setupDateInputProps() {
        ...
        let onChange;
        if (!this.props.calendarOptions.useDealFinder) {
    ...
    renderArrivalOption() {
        ...
        const destinationInputProps = {
            packageDestinations: this.props.packageDestinations,
            flightDestinations: this.props.flightDestinations,
            searchMode: currentSearchMode,
            checkValidAirport: this.isValidArrivalAirport,
            checkValidRegion: this.isValidAirportRegion,
            searchDetails: this.props.searchDetails,
            onRegionClick: this.handleResortChange,
            onAirportClick: this.props.onChange,
            getAirport: this.props.getAirport,
            getRegion: this.getRegion,
            setUseDealFinder: this.props.setUseDealFinder,
            updatePropertyFunction: this.props.updatePropertyFunction,
        };

        return (
            <div className="search-option search-arrival">
                {this.displayDestinationDropdown()
                    && <DestinationInput {...destinationInputProps} />}
    ...
    renderDateInput() {
        ...
        const calendarMessage = this.getCalendarMessage();
        if (calendarMessage !== '') {
            return (
                <DateInput {...dateInputProps}>
                    <div className="calendar-message">
                        <span className="calendar-key-item">{calendarMessage}</span>
                    </div>
                </DateInput>
            );
        } else if (this.props.calendarOptions.displayKey) {
            return (
    ...
    setDirectFlight(event) {
        ...
        this.updateProperty('Direct', value);
    }
    renderDateInputKey() {
        return (
    ...
    renderMoreDestinations() {
        const props = {
            moreDestinations: this.props.moreDestinations,
            expanded: this.state.showMoreDestinations,
        };
        return (
            <div>
                <MoreDestinations {...props} />
            </div>
        );
    }
    renderSearchForm() {
        ...
        const guestInputProps = {
            ...
            roomText: this.props.searchDetails.SearchMode === this.searchModes.FLIGHT_PLUS_HOTEL
                || this.props.searchDetails.SearchMode === this.searchModes.HOTEL ? 'Rooms' : '',
        };
        ...
        const directFlightsOnly = this.props.searchDetails.Direct;
        const checkboxProps = {
            label: 'Direct Flights Only',
            name: 'DirectFlight_check',
            value: directFlightsOnly,
            onChange: this.setDirectFlight,
            labelAttributes: {
                className: directFlightsOnly ? 'check selected' : 'check',
            },
        };
        return (
            <div className="search-form">
                <div className={searchFormContentClass}>
                    ...
                    {this.props.searchDetails.SearchMode !== this.searchModes.HOTEL
                        && <div className="search-option search-direct">
    ...
    render() {
        ...
        const mobileCollapsed = this.state.mobileCollapsed;
        const moreDestinations = this.props.moreDestinations;
        const showMoreDestinations = moreDestinations.hasOwnProperty('Include More Options')
            ? moreDestinations['Include More Options'] : false;
        let searchToolClass = 'search-tool-holder mt-3';
        if (showMoreDestinations) {
            searchToolClass += ' more-destinations';
        }
        if (mobileCollapsed) {
            ...
            <div className={containerClass}>
                ...
                <div className={searchToolClass}>
                    {!this.state.collapsed
                        && this.renderTabs()}
                    {!this.state.collapsed && !this.state.showMoreDestinations
                        && this.renderSearchForm()}
                    {!this.state.collapsed && this.state.showMoreDestinations
                        && showMoreDestinations && this.renderMoreDestinations()}
                    {this.state.collapsed
...
SearchTool.propTypes = {
    ...
    packageDestinations: React.PropTypes.object,
    flightDestinations: React.PropTypes.object,
    moreDestinations: React.PropTypes.object,
    getAirport: React.PropTypes.func,
    calendarMessages: React.PropTypes.array,
