namespace Web.Booking.Factories
{
    using System;
    ...
    using Template.Application.Interfaces.Lookup.Services;
    using Web.Template.Application.Interfaces.Models;
    using Web.Template.Application.Search.SearchModels;
    using Web.Template.Interfaces;

    using SearchModel = Web.Booking.Models.Application.SearchModel;
    ...
    public class SearchModelFactory : ISearchModelFactory
    {
        ...
        public SearchModelFactory(IAirportService airportService)
        {
        ...
        /// <param name="isPackageSearch">if set to <c>true</c> [is package search].</param>
            ...
            int flightClassId = 0)
        {
            ...
            var secondaryArrivalIDs = this.getSecondaryArrivalIds(arrivalType, arrivalId);
        ...
        private List<int> getSecondaryArrivalIds(LocationType arrivalType, int arrivalId)
        {
            var secondaryArrivalIDs = new List<int>();
            switch (arrivalType)
            {
                    case LocationType.AirportGroup:
                        secondaryArrivalIDs =
                            this.airportService.GetAirportGroupResorts().Where(agr => agr.AirportGroupId == arrivalId)
                                .Select(agr => agr.GeographyLevel3Id).ToList();
                    break;
            }

            return secondaryArrivalIDs;
        }
