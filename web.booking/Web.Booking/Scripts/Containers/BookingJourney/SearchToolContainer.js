...
class SearchToolContainer extends React.Component {
    ...
    isInitialised() {
        ...
        return this.props.search.isLoaded
            ...
            && this.props.airportGroups.isLoaded || !showAirportGroups
    ...
    componentDidMount() {
        ...
        this.setUseDealFinder(this.isDealFinderDestination(this.props.search.searchDetails));
    ...
    checkRouteChange(nextProps) {
        ...
        if (updatedSearchDetails.DepartureId && updatedSearchDetails.ArrivalId
    ...
    searchDetailsToUrl() {
        ...
        searchURL = `${searchURL}/${searchDetails.ArrivalType}/${searchDetails.ArrivalId}`;
    ...
    getSearchDestination(searchDetails) {
        const configuration = this.props.entity.model.Configuration;
        const destinations = configuration.Destinations
            ? JSON.parse(configuration.Destinations) : {};
        let searchDestinations = [];
        if (searchDetails.SearchMode === SearchConstants.SEARCH_MODES.FLIGHT_PLUS_HOTEL) {
            searchDestinations = destinations.PackageDestinations
                ? destinations.PackageDestinations : [];
        } else if (searchDetails.SearchMode === SearchConstants.SEARCH_MODES.FLIGHT) {
            searchDestinations = destinations.FlightDestinations
                ? destinations.FlightDestinations : [];
        }
        let searchDestination = null;
        searchDestinations[0].DestinationGroups.forEach(group => {
            group.Destinations.forEach(destination => {
                if (searchDestination === null
                    && destination.Region === searchDetails.ArrivalId) {
    ...
    isDealFinderDestination(searchDetails) {
        const destination = this.getSearchDestination(searchDetails);
        return this.props.site.SiteConfiguration.SearchConfiguration.UseDealFinder
            && (destination === null || !destination.ShowAllDatesAndDurations);
    }
    validateSearchDetails() {
        ...
        if ((searchDetails.SearchMode === 'Flight'
            || searchDetails.SearchMode === 'FlightPlusHotel') && !searchDetails.DepartureId) {
            isValid = false;
    ...
    validateFlightCacheResults(nextProps) {
        ...
        if (flightCache.isInitialised
            ...
            && searchDetails.ArrivalId
            && searchDetails.SearchMode !== 'Hotel') {
            const searchDestination = this.getSearchDestination(searchDetails);

            const errors = {};
            const departureDates = flightCache.departureDates;
            if ((searchDestination === null
                || (searchDestination !== null && !searchDestination.ShowAllDatesAndDurations))
    ...
    searchToolProps() {
        ...
        const resortId = UrlFunctions.getQueryStringValue('resortid');
        const details = this.props.search.searchDetails;
        ...
        const searchToolProps = {
            ...
            countries: this.props.countries.items ? this.props.countries.items : [],
            ...
            collapseOnMobile: contentModel.Configuration.CollapseOnMobile,
            sidebarDisplay: contentModel.Configuration.SidebarDisplay,
            ...
            calendarOptions: {
                ...
                displayKey: configuration.CalendarOptions
                    ? configuration.CalendarOptions.HighlightFlightCacheDates : false,
