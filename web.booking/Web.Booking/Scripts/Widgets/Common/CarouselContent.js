...
export default class CarouselContent extends React.Component {
    ...
    componentWillReceiveProps() {
        if (this.props.updateIndex) {
            this.setState({ activeItem: this.props.activeIndex },
                () => this.setActiveItems(this.props.activeIndex));
        }
    }
    handleResize() {
        const mobileDisplay = window.innerWidth < this.screenMdMin;
    ...
    renderCarouselTiledItems() {
        ...
        if (!this.state.sidePreviewDisplay) {
            carouselItemsAttributes.className = `${carouselItemsAttributes.className} container`;
        }

        const percentageMultiplier = 100;
        const rowAttributes = {
            className: this.state.sidePreviewDisplay ? '' : 'row',
            style: {
    ...
    renderCarouselItem(carouselItem, index) {
        let CarouselComponent = CarouselComponents[this.props.carouselItemType];
        if (carouselItem.hasOwnProperty('OverrideItemType')) {
            CarouselComponent = CarouselComponents[carouselItem.OverrideItemType];
        }
        ...
        const onClick = { onClick: () =>
            (this.props.updateFunction ? this.props.updateFunction(index) : undefined),
        };
        return (
            <div key={key} className={itemClass}
                    onClick={onClick.onClick}>
                <CarouselComponent
                    carouselItemHeightMobile={carouselItemHeightMobile}
    ...
    getContainerClass() {
        ...
        if (this.props.customClass !== '') {
            containerClass = `${containerClass} ${this.props.customClass}`;
        }
    ...
    getCarouselClass() {
        ...
        if (this.props.displayArrows) {
            carouselClass = `${carouselClass} arrows`;
        }
        if (this.state.sidePreviewDisplay) {
            carouselClass = `${carouselClass} side-preview`;
        }
        if (this.state.carouselTiles === 0) {
            carouselClass = `${carouselClass} full-screen`;
        }
...
CarouselContent.propTypes = {
    ...
    updateIndex: React.PropTypes.bool,
