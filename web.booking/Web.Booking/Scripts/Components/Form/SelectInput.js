...
export default class SelectInput extends React.Component {
    ...
    renderOption(option) {
        ...
        if (option && typeof option === 'object') {
            ...
            result = <option key={key} value={option.Id} className="option">{option.Name}</option>;
    ...
    render() {
        ...
        let selectContainerClass = 'select-container';
        if (this.props.selectContainerClass) {
            selectContainerClass += ` ${this.props.selectContainerClass}`;
        }
        const phSelected = this.props.disabled ? true : null;
        return (
                    ...
                    <div className="custom-select">
                        <select {...this.getInputProps()}>
                            {this.props.placeholder
                                && <option className="placeholder" disabled selected={phSelected}>
                            ...
                            {this.state.options && this.props.renderOptionGroups
                                && this.state.options.filter(o =>
                                    o.Options).map(this.renderOptionGroup, this)}
                            {this.state.options && this.props.renderOptionGroups
                                && this.props.renderOptionGroupsAndOptions
                                && this.state.options.filter(o =>
                                    !o.Options
                                    || o.Options.length === 0).map(this.renderOption, this)}
                        </select>
                    </div>
                </div>
...
SelectInput.propTypes = {
    ...
    renderOptionGroupsAndOptions: React.PropTypes.bool,
