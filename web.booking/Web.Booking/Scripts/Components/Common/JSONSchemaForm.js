...
class JSONSchemaForm extends React.Component {
        ...
        switch (type) {
            ...
            case 'Select':
                ...
                if (format === 'selectlookup') {
                    inputProps.lookup = schemaObject.format.split(',')[1];
                }
                break;
            case 'Checkbox':
            case 'Radio':
    ...
    renderArray(key, schemaObject, objectPath) {
        ...
        const definitionOverride = arrayItemSchema.definitionOverride;
        if (arrayItemSchema.hasOwnProperty('definitionOverride')) {
            definition = definitionOverride ? definitionOverride : definition;
        }
        const itemDefinition = definition ? definition : 'Item';
        ...
        return (
            <div className="form-section" key={objectKey}>
                ...
                <button type="button" className="btn btn-default btn-xs btn-array-add"
                    ...
                    <span className="fa fa-plus"></span>Add {itemDefinition}</button>
            </div>
        );
    }
            ...
            <div className="form-section-item" key={arrayItem.key}>
                ...
                <div className={contentClass}>
                    {sectionItem.tabs.length > 0
                        ...
                        <span className="fa fa-minus"></span>Remove {arrayItem.definition}</button>
                </div>
            </div>
        );
    }
