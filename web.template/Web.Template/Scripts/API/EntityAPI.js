...
class EntityAPI {
    ...
    loadEntity(site, name, context, mode) {
        ...
        const entityUrl = `/api/sitebuilder/entity/${site}/${name}/${safeUrlContext}/${mode}`;
        const fetchOptions = {
        ...
        return WebRequest.send(entityUrl, fetchOptions)
            ...
            .then(entity => {
                if (!this.entityPromises) {
                    this.entityPromises = [];
                }
                return Promise.all(this.entityPromises).then(() => entity);
            })
            .then(entity => {
                this.entityPromises = [];
                return entity;
    ...
    updateEntityValue(entity, field, value) {
        ...
        return this.entityValidator.validateEntity(updatedEntity, true);
    }
     ...
     */
    saveEntity(entity) {
        let entityUrl = `/api/sitebuilder/model/${entity.site}/${entity.name}/${entity.context}`;
    ...
    /**
     * Function for adding a new context bridge
     * @param {object} entity - The entity the bridge will apply to
     * @param {string} oldContext - The old context we want to bridge to
     * @return {object} The entity
     */
    addContextBridge(entity, oldContext) {
        let entityUrl = '/api/sitebuilder/contextBridge/';
        entityUrl += `${entity.site}/${entity.name}/${entity.context}/add`;

        const contextBridgeModel = {
            oldContext,
            currentContext: entity.context,
        };
        ...
        const fetchOptions = {
            ...
            method: 'PUT',
            credentials: 'same-origin',
            body: JSON.stringify(contextBridgeModel),
        };
    ...
    publishEntity(site, name, context, entity) {
        const entityUrl = `/api/sitebuilder/model/${site}/${name}/${context}/publish`;
        const fetchOptions = {
            headers: {
                'Content-Type': 'application/json',
            },
    ...
    deleteEntity(site, name, context) {
        const entityUrl = `/api/sitebuilder/model/${site}/${name}/${context}`;
        const fetchOptions = {
            headers: {
                'Content-Type': 'application/json',
            },
    ...
    loadEntityList(site, type) {
        let entitiesUrl = `/api/sitebuilder/entities/${site}`;
        if (type) {
    ...
    loadContextsList(site, entity, newContextModel) {
        ...
        const contextsUrl = `/api/sitebuilder/${site}/${entity}/contexts`;
    ...
    */
    loadModelsFromEntityType(site, entityType) {
        const modelsUrl = `/api/sitebuilder/models/${site}/${entityType}`;
        this.items = [];
        return WebRequest.send(modelsUrl)
            .then(response => {
                if (response.ok) {
