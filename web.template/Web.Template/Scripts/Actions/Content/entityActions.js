...
export function loadCMSModel(objectType, id) {
    ...
    return function load(dispatch) {
        ...
        return entityAPI.entityValidator.loadCMSModel(objectType, id).then(entity => {
...
/**
 * Redux Action method for loading an entity.
 * @param {object} entity - The entity to add the context bridge to
 * @param {string} currentContext - The context to map the bridge to
 * @return {object} The entity
 */
export function addContextBridge(entity, currentContext) {
    return function load(dispatch) {
        const entityAPI = new EntityAPI(entity.site);
        return entityAPI.addContextBridge(entity, currentContext).then(success => {
            if (success) {
                dispatch(loadEntity(entity.site, entity.name, entity.context, 'live'));
            }
        }).catch(error => {
            throw (error);
        });
