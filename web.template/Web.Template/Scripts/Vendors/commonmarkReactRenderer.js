...
function getNodeProps(node, key, opts, renderer) {
    ...
    switch (type) {
        ...
        case 'link':
		    ...
		    if (props.title && props.title.indexOf('|') > -1) {
                props.title = node.title.split('|')[0];
                props.target = node.title.split('|')[1];
            } else if (node.title && node.title.startsWith('_')) {
                props.title = undef;
                props.target = node.title;
            }
            break;
        case 'image':
