...
export default class SliderInput extends React.Component {
    ...
    renderPriceRange() {
        ...
        const formattedMinValue = NumberFunctions.formatPrice(rangeMinValue,
            this.props.pricingConfiguration, this.props.currency);

        const rangeMaxValue = this.state.maxControl.value + priceAdjustment;
        const formattedMaxValue = NumberFunctions.formatPrice(rangeMaxValue,
            this.props.pricingConfiguration, this.props.currency);

        let range = `${prependText} ${formattedMinValue}`;
        range += ` - ${prependText} ${formattedMaxValue}`;

        return (
            <p className="slider-range-display">{range}</p>
        );
    }
    render() {
        ...
        let containerClass = 'slider-input-container row';
        let sliderClasses = 'slider-input';

        if (this.props.showMaxPriceInput && this.props.showMinPriceInput) {
            containerClass = `${containerClass} text-inputs-visible`;
            sliderClasses
