...
/**
 * Redux Action method for resetting the email form
 * @return {object} The reset form
 */
export function resetEmailForm() {
    return { type: types.EMAILFORM_RESET };
}
...
/**
 * Redux Action method for updating the response message
  * @param {string} value - The value we're looking to update it with.
 * @return {object} The updated message
 */
export function setResponseMessage(value) {
    return { type: types.FORM_SET_MESSAGE, value };
}
...
export function resetResponseMessage() {
    return { type: types.FORM_RESET_MESSAGE };
}
