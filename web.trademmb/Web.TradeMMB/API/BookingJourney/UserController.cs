    ...
    public class UserController : ApiController
    {
        ...
        public SessionViewModel ChangeWebsite(string websiteName)
        {
            ...
            var viewModel = new SessionViewModel()
                                {
                                    UserSession = this.userService.GetUser(HttpContext.Current), 
                                    Success = true
                                };
            return viewModel;
        }
        ...
        [HttpGet]
        public async Task<SessionViewModel> TradeLogin(
            [FromUri] string username = "", 
            [FromUri] string password = "", 
            [FromUri] string websitePassword = "", 
            [FromUri] string emailAddress = "")
        {
            var loginReturn = this.tradeService.Login(username, password, websitePassword, emailAddress);
            ...
            var viewModel = new SessionViewModel()
                                {
                                    UserSession = this.userService.GetUser(HttpContext.Current), 
                                    Warnings = loginReturn.Warnings, 
                                    Success = loginReturn.LoginSuccessful
                                };
